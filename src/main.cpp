...
unsigned int nStakeMinAge = 60 * 60 * 24 * 7;	//  age for coin age: 7d
unsigned int nStakeMaxAge = 60 * 60 * 24 * 100;	// stakeminimum age of full weight: 100d
unsigned int nStakeTargetSpacing = 60;			// 60 sec block spacing

int64 nChainStartTime = 1397980276;
...
const string strMessageMagic = "BTCtalkcoin Signed Message:\n";
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
	...
	if(nHeight == 1)
	{
		nSubsidy = 31780920 * COIN;	// 50% premine
		return nSubsidy + nFees;
	}
	
	else if(nHeight <= 200)
	{
		nSubsidy = 1 * COIN;	// To avoid instamine
		return nSubsidy + nFees;
	}
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in BTCtalkcoin
    bool fStrictPayToScriptHash = true; // Always active in BTCtalkcoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BTCtalkcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        assert(block.hashMerkleRoot == uint256("0x17149fc45850ae5a4ce62657cef4c77a13974cb79b29b991b75ab047b51c6570"));
